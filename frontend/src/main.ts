...
async function main(): Promise<void> {
  ...
  bindPreviewSourceNav(async (loc) => {
    ...
    try {
      ...
      // Best-effort recovery: confirm the clicked text actually sits on the
      // located line, else find it within a few lines. Covers constructs whose
      // locator is imperfect (multi-line font-switch wrappers, macro-argument
      // columns). Falls through to the locator's own line:col when the
      // fingerprint is too short or no nearby line matches.
      const recovered = recoverSourcePosition(
        editor.getSource(),
        loc.line,
        loc.text,
        loc.word,
      ...
      if (recovered) {
        editor.revealPosition(recovered.line, recovered.col);
      } else {
